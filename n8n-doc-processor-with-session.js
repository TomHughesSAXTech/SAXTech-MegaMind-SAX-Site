...
// PRESERVES sessionId and all other fields from input

// First, capture ALL input data to preserve it
const inputJson = $input.first().json;
const sessionId = inputJson.sessionId || inputJson.session_id || inputJson.MESSAGE_METADATA?.sessionId;

// Get attachment data if it exists
const fileData = inputJson.attachments?.[0]?.data || '';
const fileName = inputJson.attachments?.[0]?.name || 'unknown';
const fileType = inputJson.attachments?.[0]?.type || '';
...
// Only process if there's actually attachment data
if (fileData) {
  try {
...
// CRITICAL: Return ALL original data plus processing results
// This ensures sessionId and all other fields are preserved
return {
  ...inputJson,  // Preserve ALL original input data including sessionId
  sessionId: sessionId || inputJson.sessionId,  // Ensure sessionId is definitely present
  attachmentProcessing: processingResult,  // Add processing results as a separate field
  // Optional: add extracted content at top level for easier access
