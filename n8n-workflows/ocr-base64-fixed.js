// OCR Processing for Screenshots and Document Attachments - Fixed Base64 Handling
...
async function extractTextFromImage(attachment) {
    try {
        ...
        const analyzeResponse = await $http.post(analyzeUrl, imageBuffer, {
            headers: {
        ...
        while (attempts < maxAttempts) {
            ...
            const resultResponse = await $http.get(operationLocation, {
                headers: {
                    'Ocp-Apim-Subscription-Key': AZURE_API_KEY
                }
            });
