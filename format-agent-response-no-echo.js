// FORMAT AGENT RESPONSE - HANDLES BOTH SEPARATE AND MERGED INPUTS WITH HTML SUPPORT
...
// NEW: Check if the AI output contains HTML (for employee cards, etc)
const hasHtml = /<[^>]+>/.test(aiOutput);
let htmlResponse = aiOutput;
let plainTextResponse = aiOutput;

// Clean up AI output for TTS (remove HTML if present)
if (hasHtml) {
  plainTextResponse = aiOutput.replace(/<[^>]+>/g, '').replace(/\s+/g, ' ').trim();
  console.log('[Format Agent Response] HTML detected, creating plain text version for TTS');
} else {
  plainTextResponse = aiOutput.trim();
}

// Set defaults if no AI output
if (!aiOutput) {
  aiOutput = "I'm ready to help. How can I assist you today?";
  plainTextResponse = aiOutput;
  htmlResponse = aiOutput;
}
...
// Prepare TTS text (using plain text version if HTML was present)
let fullResponse = aiOutput;  // This keeps the HTML for display
let ttsText = plainTextResponse;  // This is always plain text for TTS
...
if (isPreview) {
  ...
  htmlResponse = fullResponse;
}
// Handle TTS summary for long responses
else if (enableTTS && ttsSummaryLength === 'short' && plainTextResponse.length > 200) {
  const sentences = plainTextResponse.match(/[^.!?]+[.!?]+/g) || [plainTextResponse];
  
  if (sentences.length > 3) {
    ttsText = sentences.slice(0, 2).join(' ').trim();
    if (ttsText.length < plainTextResponse.length * 0.5) {
      ttsText += " I've provided more details in the text response.";
    }
  } else if (plainTextResponse.length > 250) {
    let cutoff = plainTextResponse.substring(0, 200).lastIndexOf('. ');
    if (cutoff === -1) cutoff = 200;
    ttsText = plainTextResponse.substring(0, cutoff).trim() + "... See the full response in text.";
  }
}
...
// CRITICAL FIX: Return single object, not array
// This prevents duplicate TTS generation which causes echo
return {
  // Text responses - NEW: separate HTML and plain text
  response: fullResponse,  // Original response (may contain HTML)
  text: plainTextResponse,  // Plain text version
  ttsText: enableTTS ? ttsText : '',  // TTS-optimized text
  htmlResponse: htmlResponse,  // HTML version for display
  hasHtml: hasHtml,  // Flag indicating HTML presence
  output: fullResponse,  // Backward compatibility
