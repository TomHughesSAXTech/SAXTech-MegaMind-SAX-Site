...
// Updated with better error handling for payment issues
...
try {
  ...
  const isPaymentIssue = errorMessage.includes('payment') || 
                         errorMessage.includes('past_due') ||
  ...
  let friendlyMessage = 'TTS temporarily unavailable';
  
  if (isPaymentIssue) {
    errorType = 'payment_issue';
    friendlyMessage = 'TTS subscription payment issue - audio disabled';
    console.error('ElevenLabs payment issue detected - subscription needs to be renewed');
  } else if (isQuotaError) {
    errorType = 'quota_exceeded';
    friendlyMessage = 'TTS quota exceeded - audio disabled';
  }
  
  // Pass through without audio on error - DON'T STOP THE WORKFLOW
  // The text response is still valuable even without audio
  return [{
    json: {
      ...$json,
      audioUrl: null,
      audioBase64: null,
      ttsEnabled: false,
      ttsError: friendlyMessage,
      ttsErrorType: errorType,
      ttsErrorDetail: error.message,
      // IMPORTANT: Still return the text response even if TTS fails
